...
/**
 * brevo.js — Frontend email service
 *
 * Calls the Express backend at /api.
 * The Brevo API key never lives here — it's in api/.env only.
 */
...
// Called in app.js → handleSignup, after successful signup
export async function sendWelcomeEmail(userEmail, fullName) {
  try {
    const authHeader = await getAuthHeader();
    await fetch(`${API_URL}/emails/welcome`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeader },
      body: JSON.stringify({ email: userEmail, full_name: fullName }),
    });
  } catch (err) {
    // Non-blocking — a failed welcome email should never break signup
    console.error('[brevo] Welcome email error:', err.message);
  }
}

// Called in app.js → saveApplication, when status changes to 'awaiting'
export async function sendApplicationSubmitted(userEmail, appName) {
  try {
    const authHeader = await getAuthHeader();
    await fetch(`${API_URL}/emails/application-submitted`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeader },
      // userEmail intentionally not sent — server reads it from the JWT
      body: JSON.stringify({ app_name: appName }),
    });
  } catch (err) {
    console.error('[brevo] Application submitted email error:', err.message);
  }
}

// Deadline reminders are handled server-side by api/scheduler.js
// This export exists only so app.js imports don't break
